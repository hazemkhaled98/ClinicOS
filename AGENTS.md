...
**After a phase's PR is merged:** mark that phase `done` in **both** the `## Phase Status` table in this `AGENTS.md` and the status table in `docs/roadmap.md` (same commit):
1. Update `docs/roadmap.md` status table — flip the merged phase to `done`
2. Update this `AGENTS.md` — flip the same phase to `done` in the `## Phase Status` table, and update this file if architecture/operations changed
