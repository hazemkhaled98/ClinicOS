...
| 3 — UC-003 Daily work/attendance | in progress | Slices 3a–3f: V20 work calendar (`clinic_settings.working_weekdays` weekday mask + `clinic_holiday` table), MinIO object storage + `AttachmentService`, staff attendance/daily-task/assignment services, `/employees` daily-work screen + photo/frequency task fields, admin work-calendar UI (weekday mask + holidays), A5 (offline recording) marked not applicable |
