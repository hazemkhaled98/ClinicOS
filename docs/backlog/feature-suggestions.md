...
| 4 | ~~Low~~ Done | ~~**Dynamic clinic branding**~~ — replace hardcoded `عيادتي` with the clinic name registered in sign-up | None — pass `clinic.name` through the session into templates. | Landed in Phase A (`feat/backlog-ui-polish`): `SessionKeys.CLINIC_NAME` + `LayoutModel.clinicName`. |
| 5 | ~~Medium~~ Done | ~~**Save feedback**~~ — show a visible success toast after HTMX form saves | None — reusable client-side toast component. | Landed in Phase A: audited every `ui/*Controller` mutating handler for a missing `Toasts` call. |
| 6 | ~~Medium~~ Done | ~~**Actionable empty states**~~ — link empty inventory, ordering, and academy screens to their required setup action | None. | Landed in Phase A: shared `fragments/empty-state.html` fragment. |
