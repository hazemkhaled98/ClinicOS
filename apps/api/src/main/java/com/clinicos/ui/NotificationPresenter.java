...
public class NotificationPresenter {
    ...
    public Item present(Notification notification) {
        ...
        String subject = payload.getOrDefault("task",
                payload.getOrDefault("unit", payload.getOrDefault("item",
                        payload.getOrDefault("supplier", ""))));
        return switch (kind) {
            case DAILY_TASK_APPROVED -> item(notification, "تم اعتماد مهمتك: " + subject);
            case DAILY_TASK_REJECTED -> item(notification, "تم رفض مهمتك: " + subject, payload.get("reason"));
            case TASK_ASSIGNMENT_APPROVED -> item(notification, "تم اعتماد المهمة الموكلة: " + subject);
            case TASK_ASSIGNMENT_REJECTED -> item(notification, "تم رفض المهمة الموكلة: " + subject,
                    payload.get("reason"));
            case ACADEMY_SUBMISSION_VERIFIED -> item(notification, "تم التحقق من وحدة: " + subject);
            case ACADEMY_SUBMISSION_REJECTED -> item(notification, "تم رفض وحدة: " + subject, payload.get("reason"));
            case INVENTORY_CHANGE_REQUESTED -> item(notification, "طلب تعديل جديد: " + subject);
            case SUPPLIER_RETURN_REQUESTED -> item(notification, "طلب إرجاع جديد للمورد: " + subject);
        };
    }
